...
@section('content')
    <div class="contains">
        <span>*Note: You can only make one update per day.</span>
        <h1>Edit Spa</h1>
        <div class="row">
            {{--<form method="patch" action="{{ route('store.update', $store->id)}}" enctype="multipart/form-data" class="col-md-6 create-form">--}}
            {!! Form::model($store, ['method' => 'PATCH','route' => ['store.managerUpdate', $store->id], 'class'=> 'create-form']) !!}
            ...
            <div class="input-field">
                ...
                <input type="number" min="0" name="num_rooms" id="num_rooms" class="form-control" value="{{ $store->num_rooms }}">
            </div>

            <div class="input-field">
                <label for="num_showers">Number of Showers</label>
                <input type="number" min="0" name="num_showers" id="num_showers" class="form-control" value="{{ $store->num_showers }}">
            </div>
